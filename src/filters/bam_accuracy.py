...
def _compute_accuracy_fast(read) -> Optional[float]:
    """Compute accuracy using NM tag and CIGAR ops (C-level in pysam)."""
    try:
        nm = read.get_tag("NM")
    except KeyError:
        return None
    ...
    for op, length in read.cigartuples:
        if op in (0, 1, 2):  # M, I, D
            denom += length
    if denom == 0:
        return None
    return (denom - nm) / denom
