...
class TestGenerateInputConfig:
    def test_generates_basic_config_without_master(self):
        result = generate_input_config("/input", "/output", {"norm_cutoff": 0.15})
    ...
    def test_empty_overrides_path(self):
        ...
        assert (
            ...
            or "NORM_CUTOFF_OVERRIDES" not in result
        )
    ...
    def test_writes_thread_settings_without_master(self):
        result = generate_input_config("/input", "/output", {"num_threads": 128})
    ...
    def test_appends_thread_settings_when_missing_from_master(
        self, sample_input_config
    ):
        result = generate_input_config(
            "/input",
            "/output",
    ...
    def test_enable_snv_yes(self):
        result = generate_input_config("/input", "/output", {"enable_snv": True})
    ...
    def test_enable_snv_false(self):
        result = generate_input_config("/input", "/output", {"enable_snv": False})
        assert "ENABLE_SNV=no" in result
