...
from src.pipeline.summary_generator import _normalize_counts, generate_summaries
...
class TestGenerateSummaries:
    def _setup_barcode_dir(self, tmp_dir):
        ...
        # Locus D3S1358
        with open(os.path.join(barcode_dir, "D3S1358_Allele_freqs.txt"), "w") as f:
    ...
    def test_creates_summary_tsv(self, tmp_dir):
        counting_dir = self._setup_barcode_dir(tmp_dir)
        generate_summaries(counting_dir, 0.1, {})
        summary_file = os.path.join(counting_dir, "Summaries", "barcode01_summary.tsv")
        assert os.path.isfile(summary_file)

    def test_creates_profile_tsv(self, tmp_dir):
        counting_dir = self._setup_barcode_dir(tmp_dir)
        generate_summaries(counting_dir, 0.1, {})
        profile_file = os.path.join(counting_dir, "Summaries", "barcode01_Profile.tsv")
    ...
    def test_normalization_method_total(self, tmp_dir):
        counting_dir = self._setup_barcode_dir(tmp_dir)
        result = generate_summaries(counting_dir, 0.1, {}, normalization_method="total")
        assert "barcode01" in result

    def test_stutter_filter_disabled(self, tmp_dir):
        counting_dir = self._setup_barcode_dir(tmp_dir)
        result = generate_summaries(counting_dir, 0.1, {}, stutter_filter=False)
        assert "barcode01" in result
